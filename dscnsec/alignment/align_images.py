...
while True:
    ...
    # Find all contours
    contours, hierarchy = cv2.findContours(image_dilation, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # cv2.RETR_EXTERNAL helps to find the outer edges

    # Draw all detected contours
    cv2.drawContours(image_copy, contours, -1, (0, 255, 0), 10) # DRAW ALL DETECTED CONTOURS
    ...
    if biggest.size !=0:
        ...
        cv2.circle(image_copy_2,(biggest_in_order[0][0][0],biggest_in_order[0][0][1]),5,(0,0,255),cv2.FILLED)
        cv2.circle(image_copy_2,(biggest_in_order[1][0][0],biggest_in_order[1][0][1]),5,(0,0,255),cv2.FILLED)
        cv2.circle(image_copy_2,(biggest_in_order[2][0][0],biggest_in_order[2][0][1]),5,(0,0,255),cv2.FILLED)
        cv2.circle(image_copy_2,(biggest_in_order[3][0][0],biggest_in_order[3][0][1]),5,(0,0,255),cv2.FILLED)
        ...
        imageWrapColored = cv2.warpPerspective(resized_image,matrix,(widthImg,heightImg))
    
        # Remove false positive ( 20 pixel from each side )
        imageWrapColored = imageWrapColored[20:imageWrapColored.shape[0]-20,20:imageWrapColored.shape[1]-20]
        ...
        ImageWrapThresold = cv2.adaptiveThreshold(ImageWrapGauss, 255, 1,1, 7, 2)
